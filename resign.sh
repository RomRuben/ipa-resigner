...
if [[ $codes_availables != *$SIGNING_IDENTITY* ]];
then
  echo "The signing identity not exists."
  exit 1
fi

IPA_CONTENT_PATH=$DESTINATION_PATH/ipa_content
...
#Data analysis
echo "Analysing data..."
cp $PROVISIONING_PROFILE $DESTINATION_PATH/$PROVISIONING_PROFILE
...
#Packaging
IPA_RESIGNED="$DESTINATION_PATH/RESIGNED_${APP_NAME}.ipa"
cd $IPA_CONTENT_PATH
zip --symlinks --verbose --recurse-paths ${IPA_RESIGNED} .


#Cleaning tmp files
rm $DESTINATION_PATH/$PROVISIONING_PROFILE
rm -rf $IPA_CONTENT_PATH
rm $APP_ENTITLEMENTS
